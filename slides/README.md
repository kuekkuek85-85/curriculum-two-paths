...
2. **1번 슬라이드로 돌아온 뒤** `O` → **응답 데이터 초기화** 클릭
   (초기화는 청중 참여 상태도 함께 닫습니다. 인터랙션 슬라이드에 머문 채 누르면
   그 인터랙션이 곧바로 다시 열리므로, 반드시 1번 슬라이드에서 누르세요)
3. S1을 대기 화면으로 띄워두고 청중 입장 유도
4. 네트워크 불가 시: 폴·퀴즈는 거수, S19는 지명 발언 — 슬라이드는 그대로 진행 가능

### 발표 중 안전장치

- **새로고침해도 보던 슬라이드로 돌아옵니다** (탭을 닫았다 열면 1번부터)
- 창을 하나 더 열거나 청중이 20번 슬라이드 QR로 발표자 화면을 열어도
  진행 중인 청중 참여가 끊기지 않습니다
- 세션이 어긋나면 발표자 화면이 몇 초 안에 스스로 현재 슬라이드 상태로 되돌립니다
- 네트워크가 끊기면 인터랙션 슬라이드가 "거수로 진행" 안내로 자동 전환됩니다
